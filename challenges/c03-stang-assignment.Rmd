...
- How many aluminum alloys are in this dataset? How do you know?
  - There is only one aluminum alloy in the dataset. I know this because the summary of the     dataset says that there's only one unique value in the `alloy` column.
...
- (Write your own question here)
  - Does angle have a greater effect on elasticity or Poisson's ratio in aluminum?
...
- (Address your question from q1 here)
  - The dataset doesn't give an answer to my question. My visualization shows that angle       doesn't have much of a relationship with either elasticity or Poisson's ratio in           aluminum. There are a variety of angles in each area of the plot, so no clear pattern      is established. It would be helpful to have data on elasticity and Poisson's ratio at      more angles to see if a pattern starts to emerge.
...
- Does this graph support or contradict the claim above?
  - This graph contradicts the claim above. The lowest elasticity values shown on the graph     occur specifically when aluminum has a thickness of 0.081, the largest thickness in the     dataset. The highest elasticity values generally occur when aluminum has a thickness of     0.022, the smallest thickness in the dataset. Although it is not the most definite         pattern, lower elasticity tends to correlate with greater thickness.
  
- Is this evidence *conclusive* one way or another?
  - This evidence is not conclusive. Although there is a general pattern of greater            thickness meaning lower elasticity seen for thicknesses of 0.022, 0.032, and 0.081, the     same does not apply when the thickness is 0.064. Elasticity values at this thickness       are all either equal to or higher than those at a thickness of 0.032, even though 0.064     is greater. There would have to be testing with a wider range of thickness values to       determine is elasticity is intensive or not.
