...
A line plot assumes that the data have a function relation.

- How does the `mpg` dataset violate this assumption?
The `mpg` dataset violates this assumption because the data is not 1:1. There are a variety of different "classes" that each have different `displ` and `hwy` values. 
...
The `Raw` data has alternating spikes and dips that gradually increase, then slightly decrease, and then increase again.

- What can you tell based on the `Smoothed` data?
The `Smoothed` data has a much clearer trend. It first increases, then gradually decreases before increasing again. There are no extreme spikes or dips. 
