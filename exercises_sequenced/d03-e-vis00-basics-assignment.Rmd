...
The `cut` variable specifies the quality of the diamond's cut, while the `color` variable specifies its color and the `clarity` variable specifies how clear it is.
...
- Price generally increases along with an increase in carats
- 1 carat diamonds have the widest range in prices
- Most diamonds are under 2.5 carats
- Prices for 2 carat diamonds are skewed towards the higher end of their range, while prices for 1 carat diamonds are skewed towards the lower end of their range
...
Aside from `x` and `y`, `geom_point()` also accepts alpha, colour, fill, group, shape, size, and stroke aesthetics.
...
ggplot(            # 1. Starting a ggplot
  ...
  geom_point(      # 4. Geometry
    mapping = aes( # 5. `Aes`thetic mapping
      x = cut,   # 5.1. Mapping the `x` variable
...
- Diamonds with a "fair" cut have the widest range of carat values.
- Diamonds with "fair" and "good" cuts generally have the lowest prices.
- Diamonds with lower carats generally have the lowest prices.
- Diamonds in the mid-range of carat values have higher prices than many of the outlier higher-carat diamonds in each cut category.
