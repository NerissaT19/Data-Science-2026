...
The dplyr, forcats, ggplot2, lubridate, purrr, readr, stringr, tibble, and tidyr packages are loaded.
...
`install.packages()` installs a new package, while `library()` loads a previously installed package into your notebook. You only have to call `install.packages()` once, while you have to call `library()` every time you use a new notebook.
