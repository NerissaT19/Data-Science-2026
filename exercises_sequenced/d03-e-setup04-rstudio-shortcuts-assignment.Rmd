...
* Within the script editor or a chunk
    ...
    * `Alt` + `-`
    Inserts an assignment operator


    * `Shift` + `Cmd/Ctrl` + `M`
    Inserts a pipe operator


    * `Cmd/Ctrl` + `Enter`
    Runs the current line/current selection


    * `F1` (Note: on a Mac you need to press `fn` + `F1`)
    Shows help for the function at cursor


	* `Cmd/Ctrl` + `Shift` + `C`
	  Comments or uncomments the current line/current selection
...
You can stop the code from using using esc.

### __q4__ How do you show the "Document Outline" in RStudio?
You can show the "Document Outline" using ctrl+shift+O.
