...
The mean departure delay time is 12.63907.
...
F9 is at the top of `mean` departure delay and FL and WN are at the top of `median` departrue delay.

- Which carriers differ in the two lists?
EV and YV are in the list that is sorted based on mean. They are replaced by UA and VX in the list that is sorted based on median. 

- What do negative values of `dep_delay` mean?
Negative values of `dep_delay` mean that a departure was early. 
...
test_iqr <- v_test_iqr %>%
  quantile(., probs = c(0, 0.25, 0.50, 0.75, 1.00)) %>%
  IQR()
```
...
HA is at the top of `sd` departure delay, while EV and YV are at the top of the `IQR` departure delay. 

- Which carriers differ in the two lists?
HA is the list that is sorted by `sd`. It is replaced by 9E in the list that is sorted by `IQR`.

- Which carrier has the biggest difference between its `sd` and `IQR`? What might this mean?
HA has the biggest difference between its `sd` and `IQR`. This might mean that it has outlier values where a departure delay was much longer than usual. 
...
```{r ex-cor-line}
slope <- -5 # Play with this value; observe the correlation
df_line <-
...
The correlation is positive when the value of `slope` is positive.

- For what values of `slope` is the correlation negative?
The correlation is negative when the value of `slope` is negative.
...
I predict that the Pearson correlation will be close to 0 and the Spearman correlation will be close to -1.

- What are the actual correlations?
The actual Pearson correlation is a negative number very close to zero and the actual Spearman correlation is -0.0235797.
