...
Both lines of code do the same thing, as they filter the diamonds dataset to only include diamonds with an "ideal" cut. However, the second version uses a pipe to insert the dataset into the filter() function, while the first version does not.
...
The result was originally the mean price of every diamond in the dataset, but after uncommenting the group_by() line, the result ends up being a list of the mean price of the diamonds in different color and clarity groups.
...
df_q4 <-
  ...
  mutate(`cov_depth` = sd(depth) / mean(depth)) %>%
  mutate(`cov_diff` = sd(diff, na.rm = TRUE) / mean(diff, na.rm = TRUE)) %>%
  select(`cov_depth`, `cov_diff`, everything())
df_q4
```
...
- Comment on the relative values of `cov_depth` and `cov_diff`.
  - `cov_depth` has a very small value below 1, while `cov_diff` has a large value, at          almost 500. 
  
- Why is `cov_diff` so large?
    - `cov_diff` is so large because there is a lot of variation in the differences between      `depth` and `computed_depth` for each row, especially because the difference can be         negative. On the other hand, there are no negative values in `cov_depth` and most          values are relatively close to each other.
...
df_q5 <- diamonds %>%
  ...
  mutate(`price_sd` = sd(price)) %>%
  mutate(`price_cov` = price_sd / price_mean) %>%
  select(`price_cov`, everything())
df_q5
```
...
**Observations**:
- "Ideal" cuts have the largest coefficient of variation for price.
- "Fair" cuts have the lowest coefficient of variation for price.
- The coefficient of variation for price increases as the cut quality increases.

These observations make sense with the context of my earlier viewings of the dataset. Diamonds with lower quality cuts skewed towards having lower prices. Diamonds of higher quality cuts still had the lower prices, but they had a greater number of options with higher prices, leading to more variation. 
