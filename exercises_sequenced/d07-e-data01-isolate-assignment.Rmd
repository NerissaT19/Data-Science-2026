...
The code only had one equal sign inside the filter() function instead of two and string value "LAX" had no quotations around it. It is trying to find all of the flights with a destination of LAX.
...
flights %>% 
  filter(dest == BOS)
```
The user created a variable named BOS that holds the string "LGA." Since they didn't put quotations around BOS in the filter() function, it used the variable rather than the string and looked for flights with a destination of LGA, not BOS.
...
- Was this dataset assembled to study flights *out of* NYC, or *into* NYC? How do you know?
  - This dataset was assembled to study flights out of NYC. You can tell because the filtered data for flights out of NYC has the same number of rows as the original dataset, while there is only one flight into NYC.
