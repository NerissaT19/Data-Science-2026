...
The vignettes available are: colwise, base, grouping, dplyr, programming, recoding-replacing, rowwise, two-table in-packages, and window-functions. 
...
To look up a specific function, we can write a ? and then the name of the function.
...
`cut_number` comes from the ggplot2 package.
...
`row_number` comes from the dplyr package.
