...
**Observations**:
Hypothesis 1 was not correct. `Ideal` cut diamonds are actually the most common cut in the data set.
...
**Observations**:
The graph does not relate to hypothesis 2. It is hard to read since there are many data points that are the same color and overlapping. It shows price ranges for each type of cut, but you can't tell how frequent each price is relative to other cuts.
...
**Observations**:
The graph contradicts hypothesis 2. `Ideal` cut diamonds have the lowest median price out of any of the cut types, and their upper price quartile is also the lowest of the cuts.
...
**Observations**:
The graph supports a "carat over cut" hypothesis. Diamonds with a `Fair` cut have the highest median carat value, and the median carat value generally decreases as the cut becomes higher quality, aside from an increase for `Premium` cuts. This might mean that carat is more important for pricing than cut, because cut types that have a higher median for carat values have a higher median for price values.
