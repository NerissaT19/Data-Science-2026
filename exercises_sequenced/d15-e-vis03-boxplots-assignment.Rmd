...
The minivan class tends to be the most "middle" for `displ` values. 

- Which plot is more effective for answering the previous question?
The boxplot is more effective for answering the previous question. 
...
diamonds %>%
  ...
  mutate(carat_cut = cut_interval(1:53940, 15)) %>%
  ggplot(aes(x = carat_cut, y = price)) +
  geom_boxplot()
```

**Observations**
- When the cut only has a length of 1, the top whisker of the box is much longer than the bottom whisker and there are many outlier values.
- No matter the cut length, the middle box always has the widest range in values between the lower and upper quartiles.
- No matter the cut length, there is a trend of the whiskers getting progressively larger up until the middle box. After this box, the whiskers become smaller and the pattern resets. The same trend applies for the medians and lower and upper quartiles.
- At greater cut lengths, the outlier values are always below the lower whiskers of the boxes. However, at lengths 10 and below, there is sometimes a line of outlier values above the upper whisker for the box after the middle box.
